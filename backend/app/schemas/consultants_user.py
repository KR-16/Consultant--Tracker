...
class ConsultantsUserSchema(CollectionSchema):
    """
    Schema definition for the consultants user collection.
    
    This is for consultant users (authentication), separate from consultant profiles.
    Note: Consultant profiles are stored in the 'consultant_profiles' collection.
    
    Defines indexes for:
    - email (unique) - for fast lookups and uniqueness constraint
    - is_active - for filtering active/inactive consultants
    """
    
    @staticmethod
    def get_collection_name() -> str:
        """Return the MongoDB collection name for consultant users"""
        return "consultants"
    
    @staticmethod
    async def create_indexes(db) -> None:
        """
        Create indexes for the consultants user collection.
        
        Indexes:
        - email: Unique index for email lookups and uniqueness constraint
        - is_active: Index for filtering active/inactive consultants
        """
        collection_name = ConsultantsUserSchema.get_collection_name()
        ...
        try:
            ...
            try:
                result = await db.consultants.create_index("email", unique=True)
            ...
            try:
                result = await db.consultants.create_index("is_active")
