import pytest
import pytest_asyncio
from httpx import AsyncClient
from app.main import app
from app.db import init_db, get_database
from app.repositories.consultants import ConsultantRepository
from app.repositories.submissions import SubmissionRepository
from app.models import ConsultantCreate, SubmissionCreate
from datetime import datetime, timedelta

@pytest_asyncio.fixture
async def client():
    """Create test client"""
    async with AsyncClient(app=app, base_url="http://test") as ac:
        yield ac

@pytest_asyncio.fixture
async def setup_test_db():
    """Setup test database"""
    await init_db()
    yield
    # Cleanup can be added here if needed

@pytest_asyncio.fixture
async def sample_consultant():
    """Create a sample consultant for testing"""
    consultant_repo = ConsultantRepository()
    consultant_data = ConsultantCreate(
        name="Test Consultant",
        experience_years=5,
        tech_stack=["Python", "FastAPI"],
        available=True,
        location="Test City",
        visa_status="CITIZEN",
        rating="GOOD",
        email="test@example.com",
        phone="+1-555-0123",
        notes="Test consultant for unit tests"
    )
    consultant = await consultant_repo.create(consultant_data)
    return consultant

class TestConsultants:
    """Test cases for consultant endpoints"""
    
    async def test_create_consultant(self, client: AsyncClient, setup_test_db):
        """Test creating a new consultant"""
        consultant_data = {
            "name": "John Doe",
            "experience_years": 3,
            "tech_stack": ["React", "Node.js"],
            "available": True,
            "location": "New York",
            "visa_status": "H1B",
            "rating": "GOOD",
            "email": "john.doe@test.com",
            "phone": "+1-555-0123",
            "notes": "Test consultant"
        }
        
        response = await client.post("/api/consultants/", json=consultant_data)
        assert response.status_code == 200
        
        data = response.json()
        assert data["name"] == consultant_data["name"]
        assert data["email"] == consultant_data["email"]
        assert data["experience_years"] == consultant_data["experience_years"]
    
    async def test_get_consultants(self, client: AsyncClient, setup_test_db, sample_consultant):
        """Test getting all consultants"""
        response = await client.get("/api/consultants/")
        assert response.status_code == 200
        
        data = response.json()
        assert len(data) >= 1
        assert any(consultant["name"] == "Test Consultant" for consultant in data)
    
    async def test_get_consultant_by_id(self, client: AsyncClient, setup_test_db, sample_consultant):
        """Test getting a specific consultant by ID"""
        response = await client.get(f"/api/consultants/{sample_consultant.id}")
        assert response.status_code == 200
        
        data = response.json()
        assert data["id"] == sample_consultant.id
        assert data["name"] == sample_consultant.name
    
    async def test_update_consultant(self, client: AsyncClient, setup_test_db, sample_consultant):
        """Test updating a consultant"""
        update_data = {
            "name": "Updated Test Consultant",
            "experience_years": 6
        }
        
        response = await client.put(f"/api/consultants/{sample_consultant.id}", json=update_data)
        assert response.status_code == 200
        
        data = response.json()
        assert data["name"] == update_data["name"]
        assert data["experience_years"] == update_data["experience_years"]
    
    async def test_delete_consultant(self, client: AsyncClient, setup_test_db, sample_consultant):
        """Test deleting a consultant"""
        response = await client.delete(f"/api/consultants/{sample_consultant.id}")
        assert response.status_code == 200
        
        # Verify consultant is deleted
        response = await client.get(f"/api/consultants/{sample_consultant.id}")
        assert response.status_code == 404

class TestSubmissions:
    """Test cases for submission endpoints"""
    
    async def test_create_submission(self, client: AsyncClient, setup_test_db, sample_consultant):
        """Test creating a new submission"""
        submission_data = {
            "consultant_id": sample_consultant.id,
            "client_or_job": "Test Company - Developer",
            "recruiter": "Test Recruiter",
            "submitted_on": datetime.now().isoformat(),
            "status": "SUBMITTED",
            "comments": "Test submission"
        }
        
        response = await client.post("/api/submissions/", json=submission_data)
        assert response.status_code == 200
        
        data = response.json()
        assert data["client_or_job"] == submission_data["client_or_job"]
        assert data["consultant_id"] == submission_data["consultant_id"]
    
    async def test_get_submissions(self, client: AsyncClient, setup_test_db, sample_consultant):
        """Test getting all submissions"""
        # First create a submission
        submission_repo = SubmissionRepository()
        submission_data = SubmissionCreate(
            consultant_id=sample_consultant.id,
            client_or_job="Test Company - Developer",
            recruiter="Test Recruiter",
            submitted_on=datetime.now(),
            status="SUBMITTED",
            comments="Test submission"
        )
        await submission_repo.create(submission_data)
        
        response = await client.get("/api/submissions/")
        assert response.status_code == 200
        
        data = response.json()
        assert len(data) >= 1
    
    async def test_update_submission_status(self, client: AsyncClient, setup_test_db, sample_consultant):
        """Test updating submission status"""
        # First create a submission
        submission_repo = SubmissionRepository()
        submission_data = SubmissionCreate(
            consultant_id=sample_consultant.id,
            client_or_job="Test Company - Developer",
            recruiter="Test Recruiter",
            submitted_on=datetime.now(),
            status="SUBMITTED",
            comments="Test submission"
        )
        submission = await submission_repo.create(submission_data)
        
        # Update status
        status_update = {
            "status": "INTERVIEW",
            "comments": "Status updated to interview"
        }
        
        response = await client.put(
            f"/api/submissions/{submission.id}/status",
            json=status_update,
            params={"changed_by": "Test User"}
        )
        assert response.status_code == 200
        
        data = response.json()
        assert data["status"] == status_update["status"]

class TestReports:
    """Test cases for reports endpoints"""
    
    async def test_status_report(self, client: AsyncClient, setup_test_db, sample_consultant):
        """Test status report endpoint"""
        # Create some submissions with different statuses
        submission_repo = SubmissionRepository()
        
        submissions_data = [
            SubmissionCreate(
                consultant_id=sample_consultant.id,
                client_or_job="Company A - Developer",
                recruiter="Recruiter A",
                submitted_on=datetime.now(),
                status="SUBMITTED",
                comments="Test submission 1"
            ),
            SubmissionCreate(
                consultant_id=sample_consultant.id,
                client_or_job="Company B - Developer",
                recruiter="Recruiter B",
                submitted_on=datetime.now(),
                status="INTERVIEW",
                comments="Test submission 2"
            )
        ]
        
        for submission_data in submissions_data:
            await submission_repo.create(submission_data)
        
        response = await client.get("/api/reports/status")
        assert response.status_code == 200
        
        data = response.json()
        assert len(data) >= 2
        
        # Check that we have the expected statuses
        statuses = [item["status"] for item in data]
        assert "SUBMITTED" in statuses
        assert "INTERVIEW" in statuses

if __name__ == "__main__":
    pytest.main([__file__])
