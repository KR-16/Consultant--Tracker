...
class ConsultantRepository:
    ...
    async def create(self, consultant: ConsultantCreate) -> Consultant:
        ...
        consultant_dict = consultant.dict()
        consultant_dict["created_at"] = datetime.now()
        consultant_dict["updated_at"] = datetime.now()
        
        result = await db[self.collection_name].insert_one(consultant_dict)
        consultant_dict["id"] = str(result.inserted_id)
        return Consultant(**consultant_dict)

    async def get_by_id(self, consultant_id: str) -> Optional[Consultant]:
        db = await get_database()
        consultant = await db[self.collection_name].find_one({"_id": ObjectId(consultant_id)})
        if consultant:
            consultant["id"] = str(consultant["_id"])
            return Consultant(**consultant)
        return None

    async def get_all(self, filters: Optional[ConsultantFilters] = None, skip: int = 0, limit: int = 100) -> List[Consultant]:
        db = await get_database()
        query = {}
        ...
        if filters:
            ...
            if filters.visa_status:
                query["visa_status"] = filters.visa_status
            if filters.rating:
                query["rating"] = filters.rating
            if filters.experience_min is not None or filters.experience_max is not None:
                experience_query = {}
                if filters.experience_min is not None:
                    experience_query["$gte"] = filters.experience_min
                if filters.experience_max is not None:
                    experience_query["$lte"] = filters.experience_max
                query["experience_years"] = experience_query

        cursor = db[self.collection_name].find(query).skip(skip).limit(limit)
        consultants = []
        async for consultant in cursor:
            consultant["id"] = str(consultant["_id"])
            consultants.append(Consultant(**consultant))
        return consultants

    async def update(self, consultant_id: str, consultant_update: ConsultantUpdate) -> Optional[Consultant]:
        db = await get_database()
    ...
    async def delete(self, consultant_id: str) -> bool:
        db = await get_database()
        result = await db[self.collection_name].delete_one({"_id": ObjectId(consultant_id)})
        return result.deleted_count > 0

    async def get_by_email(self, email: str) -> Optional[Consultant]:
        db = await get_database()
        consultant = await db[self.collection_name].find_one({"email": email})
        if consultant:
            consultant["id"] = str(consultant["_id"])
            return Consultant(**consultant)
        return None

    async def count(self, filters: Optional[ConsultantFilters] = None) -> int:
        db = await get_database()
        query = {}
